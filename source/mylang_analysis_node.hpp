#ifndef MYLANG_ANALYSIS_UNPACK_HPP
#define MYLANG_ANALYSIS_UNPACK_HPP
...
class CodeMP: public Code {
private:
    const myparser::Node<> *entry;

public:
    CodeMP(const myparser::Node<> *node): entry(node) {}
};
...
namespace noderun {
    ...
    #define MYLANG_NODE_LIST(name, len, index) \
        const myparser::NodeTypedList<MP_STR(name, len), index> *node

    #define MYLANG_NODE_TEXT(name, len) \
        const myparser::NodeTypedText<MP_STR(name, len)> *node

    inline long run(
        MYLANG_NODE_TEXT("integer", 7)
    ) {
        return node->getData(); // TODO
    }

    #undef MYLANG_NODE_LIST
    #undef MYLANG_NODE_TEXT
...
class InfoFrame<OUT(const T *, IN...)> {
private:
    using SelfType = InfoFrame<OUT(const T *, IN...)>;

    // TODO: thread_local ???
    static thread_local SelfType *instance = nullptr;
    SelfType *holding;

    std::function<void (const T *)> exec;
    OUT result;
...
public:
    inline InfoFrame(const IN... in): holding(instance) {
        instance = this;

        exec = [&](const T *node) {
            result = noderun::run(node, in...);
            instance = holding;
        };
    }
...
//     void run(const NodeTypedError<N, E> *node) {
//         // TODO: never reach
//         (void) node;
//     }
